...
PYTHON = sudo /home/robbiemcgugan/anaconda3/envs/venv/bin/python
PYTHON_OPTS =
...
swat-s1-init:
	cd swat; $(PYTHON) $(PYTHON_OPTS) init.py; cd ../..

swat-s1:
	sudo pkill  -f -u root "python -m cpppo.server.enip"; sudo mn -c
	cd swat; $(PYTHON) $(PYTHON_OPTS) run.py; cd ..

test-swat-s1:
	cd swat; $(TESTER) $(TESTER_OPTS) tests.py; cd ../..


reload-rl-envs:
	cd rl_envs; $(PYTHON) $(PYTHON_OPTS) setup.py install; cd ..
